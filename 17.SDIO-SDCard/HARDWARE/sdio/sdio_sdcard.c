...
SD_Error SD_Init(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_TRANSFER_CLK_DIV; //此处SDIO_TRANSFER_CLK_DIV为1  
  ...
  if (errorstatus == SD_OK)
  {
  	//对全局变量SDCardInfo进行了赋值
    errorstatus = SD_GetCardInfo(&SDCardInfo);	 //用来读取csd/cid寄存器，该寄存器中存放卡的性能、容量等信息
  }
...
SD_Error SD_PowerON(void)
{
  ...
  SDIO_SetPowerState(SDIO_PowerState_ON);//是SDIO上电

  /*!< Enable SDIO Clock */
  SDIO_ClockCmd(ENABLE);  	//使能外设时钟
   
   /*下面发送一系列命令,开始卡识别流程*/
  /*!< CMD0: GO_IDLE_STATE 重置所有卡为空闲状态---------------------------------*/
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_GetPowerState() == SDIO_PowerState_OFF) //判断是否已经上电
  {
  ...
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)//判断卡的类型
  {
	...
	//返回CID寄存器内容，CID寄存器的内容包括厂商ID，卡的名称，生产日期
    SDIO_CmdInitStructure.SDIO_Argument = 0x0;
  ...
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)	//不是使用2.0的卡
  {
    RCA = rca;
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
 ...
 /*********************************************************************************/
  ...
  SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);//数据传输结束产生中断
  SDIO_DMACmd(ENABLE);  //SDIO的DMA使能 
  SD_DMA_TxConfig((uint32_t *)writebuff, (NumberOfBlocks * BlockSize));	//配置DMA传输的控制信息
