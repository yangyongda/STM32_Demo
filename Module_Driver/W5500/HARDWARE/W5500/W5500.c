...
{
	W5500_CS = 0;//置W5500的SCS为低电平

	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(FDM1|RWB_WRITE|COMMON_R);//通过SPI写控制字节,1个字节数据长度,写数据,选择通用寄存器
	SPI2_ReadWriteByte(dat);//写1个字节数据

	W5500_CS = 1; //置W5500的SCS为高电平
}
...
void Write_W5500_2Byte(u16 reg, u16 dat)
{
	...
	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(FDM2|RWB_WRITE|COMMON_R);//通过SPI写控制字节,2个字节数据长度,写数据,选择通用寄存器
	SPI2_ReadWriteByte((dat>>8)&0xFF);//写16位数据
...
void Write_W5500_nByte(u16 reg, u8 *dat_ptr, u16 size)
{
	...
	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(VDM|RWB_WRITE|COMMON_R);//通过SPI1写控制字节,N个字节数据长度,写数据,选择通用寄存器

	for(i=0;i<size;i++)//循环将缓冲区的size个字节数据写入W5500
	{
		SPI2_ReadWriteByte(*dat_ptr++);//写一个字节数据
	}
...
void Write_W5500_SOCK_1Byte(SOCKET s, u16 reg, u8 dat)
{
	...
	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(FDM1|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,1个字节数据长度,写数据,选择端口s的寄存器
	SPI2_ReadWriteByte(dat);//写1个字节数据
...
void Write_W5500_SOCK_2Byte(SOCKET s, u16 reg, u16 dat)
{
	...
	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(FDM2|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,2个字节数据长度,写数据,选择端口s的寄存器
	SPI2_ReadWriteByte((dat>>8)&0xFF);//写16位数据
	SPI2_ReadWriteByte(dat&0xFF);
...
void Write_W5500_SOCK_4Byte(SOCKET s, u16 reg, u8 *dat_ptr)
{
	...
	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(FDM4|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,4个字节数据长度,写数据,选择端口s的寄存器

	SPI2_ReadWriteByte(*dat_ptr++);//写第1个字节数据
	SPI2_ReadWriteByte(*dat_ptr++);//写第2个字节数据
	SPI2_ReadWriteByte(*dat_ptr++);//写第3个字节数据
	SPI2_ReadWriteByte(*dat_ptr++);//写第4个字节数据
...
u8 Read_W5500_1Byte(u16 reg)
{
	...
	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(FDM1|RWB_READ|COMMON_R);//通过SPI1写控制字节,1个字节数据长度,读数据,选择通用寄存器

	//i=SPI_I2S_ReceiveData(SPI1);
	data = SPI2_ReadWriteByte(0x00);//发送一个哑数据
...
u8 Read_W5500_SOCK_1Byte(SOCKET s, u16 reg)
{
	...
	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(FDM1|RWB_READ|(s*0x20+0x08));//通过SPI写控制字节,1个字节数据长度,读数据,选择端口s的寄存器
...
u16 Read_W5500_SOCK_2Byte(SOCKET s, u16 reg)
{
	...
	SPI2_ReadWriteByte((reg>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(reg&0xFF);
	SPI2_ReadWriteByte(FDM2|RWB_READ|(s*0x20+0x08));//通过SPI1写控制字节,2个字节数据长度,读数据,选择端口s的寄存器

	data = SPI2_ReadWriteByte(0x00);//读取高位数据
	data *= 256;
	data += SPI2_ReadWriteByte(0x00);//读取低位数据
...
u16 Read_SOCK_Data_Buffer(SOCKET s, u8 *dat_ptr)
{
	...
	SPI2_ReadWriteByte((offset>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(offset&0xFF);
	SPI2_ReadWriteByte(VDM|RWB_READ|(s*0x20+0x18));//写控制字节,N个字节数据长度,读数据,选择端口s的寄存器
	...
	if((offset+rx_size)<S_RX_SIZE)//如果最大地址未超过W5500接收缓冲区寄存器的最大地址
	{
		for(i=0;i<rx_size;i++)//循环读取rx_size个字节数据
		{
			...
			*dat_ptr=data;//将读取到的数据保存到数据保存缓冲区
	...
	else//如果最大地址超过W5500接收缓冲区寄存器的最大地址
	{
		...
		SPI2_ReadWriteByte(0x00);//写16位地址
...
void Write_SOCK_Data_Buffer(SOCKET s, u8 *dat_ptr, u16 size)
{
	...
	SPI2_ReadWriteByte((offset>>8)&0xFF);//通过SPI写16位寄存器地址
	SPI2_ReadWriteByte(offset&0xFF);
	SPI2_ReadWriteByte(VDM|RWB_WRITE|(s*0x20+0x10));//写控制字节,N个字节数据长度,写数据,选择端口s的寄存器

	if((offset+size)<S_TX_SIZE)//如果最大地址未超过W5500发送缓冲区寄存器的最大地址
	{
		for(i=0;i<size;i++)//循环写入size个字节数据
		{
			SPI2_ReadWriteByte(*dat_ptr++);//写入一个字节的数据		
		}
	}
...
{
	W5500_RESET = 0;//复位引脚拉低
	delay_ms(50);
	W5500_RESET = 1;//复位引脚拉高
	delay_ms(200);
